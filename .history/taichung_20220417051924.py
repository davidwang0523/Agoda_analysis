...
staytime200=[None]*200
for i in range(200):
  staytime200[i]=staytime[i]
plt.plot(staytime200, label='origin')
plt.show()
# Simple Exponential Smoothing
# fit1 = SimpleExpSmoothing(staytime).fit(smoothing_level=0.2, optimized=False)
# # plot
# l1, = plt.plot(len(staytime)+5,list(fit1.fittedvalues) + list(fit1.forecast(5)), marker='o')


# fit2 = SimpleExpSmoothing(staytime).fit(smoothing_level=0.6, optimized=False)
# # plot
# l2, = plt.plot(len(staytime)+5,list(fit2.fittedvalues) + list(fit2.forecast(5)), marker='o')


# fit3 = SimpleExpSmoothing(staytime).fit()
# # plot
# l3, = plt.plot(len(staytime)+5,list(fit3.fittedvalues) + list(fit3.forecast(5)), marker='o')

# l4, = plt.plot(len(staytime)+5,staytime, marker='o')
