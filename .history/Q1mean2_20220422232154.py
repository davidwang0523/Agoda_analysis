...
for i in range(len(datacount)):
    ...
    if str(apprix_1.iloc[0, 2]).find('位於市中心'):
        distance=0
    elif str(apprix_1.iloc[0, 2]).find('查看地圖'):
        continue
    else:  # 使用爬蟲到的距離
        num = re.sub(u"([^\u0030-\u0039\u002e\uffe5])",
...
plt.title('minmaxscaler2 data')
plt.savefig('minmaxscaler2.jpg')
...
# 視覺化
# print(centroids)
originalkmeans2 = pd.DataFrame(columns=['飯店名稱', '分群'])
for i in range(len(annotations)):
    originalkmeans2 = originalkmeans2.append(
        {'飯店名稱': annotations[i], '分群': '第'+str(kmeans.labels_[i])+'群'}, ignore_index=True)
originalkmeans2.to_csv('originkmeans2.csv',encoding='utf_8_sig')
print(kmeans.labels_)
print(kmeans.labels_[355])
plt.figure(figsize=(10, 8))
plt.xlabel('distanceofcitycenter')
plt.ylabel('hot spot')
...
plt.title('originalkmeans2')
plt.savefig('originalkmeans2.jpg')

# 分群
kmeans = KMeans(n_clusters=3).fit(processdf)
# 質心
centroids = kmeans.cluster_centers_
# 視覺化
# print(centroids)
...
plt.scatter(processdf[:, 0], processdf[:, 1],
            c=kmeans.labels_.astype(float), s=50, alpha=0.5)
plt.scatter(centroids[:, 0], centroids[:, 1], c='red', s=50)
plt.title('minmaxscalekmeans2')
plt.savefig('minmaxscalekmeans2.jpg')
