...
def hotelinfocrawler(hotelid):
    frame = pd.DataFrame(columns=['飯店名稱', '房客名稱', '房客評論', '評分', '停留時間','checkin'])
    count = 0
    hotelname = ""
    for j in range(50):
        try:
        ...
        for i in range(len(final['commentList']['comments'])):
            ...
            if (final['commentList']['comments'][i]['translateSource'] == 'zh-TW') and (
                ...
                temp2 = pd.DataFrame({'飯店名稱': hotelname,
                                      ...
                                      '停留時間': [str([final['commentList']['comments'][i]['reviewerInfo']['lengthOfStay']])],
                                      'checkin': [str([final['commentList']['comments'][i]['checkInDate']])]})
...
finalframe = pd.DataFrame(columns=['飯店名稱', '房客名稱', '房客評論', '評分', '停留時間','checkin'])
finalcount = 0
for i in range(20):
    tempmultivalue = hotelinfocrawler(x.iloc[i, 1].item())
    finalcount = finalcount + tempmultivalue[1]
    finalframe = finalframe.append(tempmultivalue[0], ignore_index=True)

print(finalcount)
print(finalframe)
finalframe.to_csv("alltaichungdata.csv", encoding='utf_8_sig')

# test unit
# list = [1220620, 16362142, 2181008, 908412, 2089511]
# for i in range(len(list)):
#     tempmultivalue = hotelinfocrawler(list[i])
#     finalcount = finalcount + tempmultivalue[1]
#     finalframe = finalframe.append(tempmultivalue[0], ignore_index=True)

# print(finalcount)
# print(finalframe)
# finalframe.to_csv("checkerror.csv", encoding='utf_8_sig')
