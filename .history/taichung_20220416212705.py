...
while True:
    ...
    while True:
        ...
        for i in range(len(y)):
            if scrollcount == 0:
                temp_scroll[scrollcount] = temp_scroll[scrollcount].append(y[i].text)
                print(temp_scroll[scrollcount])
                #第一次的滾動結果加入暫存
                break
            else: # 若不是第一次 則加上上次滾動的結果到下一列
                temp_scroll[scrollcount] = temp_scroll[scrollcount-1].append(y[i].text)
        scrollcount += 1
