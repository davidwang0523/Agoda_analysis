...
def hotelinfocrawler(hotelid, distance):
    ...
    if count != 0:  # 若評論數量不為0則append
        # 抓取指定飯店的人氣景點距離
        secondaryscription = 'https://www.agoda.com/api/cronos/property/BelowFoldParams/GetSecondaryData?finalPriceView=1&isShowMobileAppPrice=false&cid=1891473&numberOfBedrooms=&familyMode=false&adults=2&children=0&rooms=1&maxRooms=0&checkIn=2022-04-20&isCalendarCallout=false&childAges=&numberOfGuest=0&missingChildAges=false&travellerType=1&showReviewSubmissionEntry=false&currencyCode=TWD&isFreeOccSearch=false&tag=c6cac438-31ac-9b9f-bc08-0b36bc6fdc3f&flexibleDateSearchCriteria=%5bobject+Object%5d&isCityHaveAsq=false&los=1&searchrequestid=cddcd015-1c3e-47bf-bddd-026cc744ab14&hotel_id='+str(hotelid)+'&all=false&price_view=1&sessionid=sfvxh2gj4e0mtssobk0kynww&pagetypeid=7'
        r1 = requests.get(secondaryscription, headers=secondaryheaders)
        sleep(2)
        temp = r1.json()
        print(temp['translations']['placesOfInterest']['placesOfInterestProperties'])
        sum = 0
        for i in range(len(temp['translations']['placesOfInterest']['placesOfInterestProperties'])):
            print(temp['translations']['placesOfInterest']['placesOfInterestProperties'][i]['places']['name'])
            print(temp['translations']['placesOfInterest']['placesOfInterestProperties'][i]['places']['distance'])
