...
x = df['距離市中心'].to_list()
y = df['人氣景點平均距離'].to_list()
plt.figure(figsize=(10, 8))
plt.xlabel('distanceofcitycenter')
plt.ylabel('hot spot')
plt.scatter(df['距離市中心'], df['人氣景點平均距離'], c='red', s=50)
for count in range(len(df['距離市中心'])):
    x[i]=int(x[i])
    y[i]=int(y[i])
plt.xticks(df['距離市中心'].to_list())
plt.yticks(df['人氣景點平均距離'].to_list())
for i, label in enumerate(annotations):
    plt.annotate(label, (df.iloc[i, 0], df.iloc[i, 1]))
plt.show()
