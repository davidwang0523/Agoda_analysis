...
def picture(staytime, n):
    ...
    fit1 = SimpleExpSmoothing(reversed(staytimetemp)).fit(
        smoothing_level=0.2, optimized=False)
    yhat = fit1.predict(len(reversed(staytimetemp)))
    print(yhat)
    # plot
    l1, = plt.plot(list(fit1.fittedvalues) +
                   list(fit1.forecast(5)), marker='o')

    fit2 = SimpleExpSmoothing(reversed(staytimetemp)).fit(
        smoothing_level=0.6, optimized=False)
    yhat = fit2.predict(len(reversed(staytimetemp)))
    print(yhat)
    # plot
    l2, = plt.plot(list(fit2.fittedvalues) +
                   list(fit2.forecast(5)), marker='o')

    fit3 = SimpleExpSmoothing(reversed(staytimetemp)).fit()
    # plot
    yhat = fit3.predict(len(reversed(staytimetemp)))
    print(yhat)
    l3, = plt.plot(list(fit3.fittedvalues) +
                   list(fit3.forecast(5)), marker='o')

    l4, = plt.plot(reversed(staytimetemp), marker='o')
