...
resultsx = list(map(float, df['距離市中心'].to_list()))
resultsy = list(map(float, df['人氣景點平均距離'].to_list()))
plt.xticks(resultsx,fontsize=(8),rotation=90)
plt.yticks(resultsy,fontsize=(8))
for i, label in enumerate(annotations):
    if resultsx[i]>50 or resultsy[i]>50:
     plt.annotate(label, (resultsx[i], resultsy[i]))
