...
# 標準化
scaler = preprocessing.MinMaxScaler(feature_range=(0, 1)).fit(df['平均停留時間'])
X_scaled = scaler.transform(df['平均停留時間'])
scaler = preprocessing.MinMaxScaler(feature_range=(0, 1)).fit(df['距離市中心'])
Y_scaled = scaler.transform(df['距離市中心'])
plt.figure(figsize=(10, 8))
plt.xlabel('avgstaytime')
plt.ylabel('distance')
plt.scatter(X_scaled, Y_scaled, c='red', s=50)
