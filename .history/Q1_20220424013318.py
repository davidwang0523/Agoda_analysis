...
s = SnowNLP(x[3])
print(commentsentiment(s))
datascore = pd.DataFrame(columns=['checkin', 'score'])
for i in range(30):
    s = SnowNLP(x[i])
    datascore=datascore.append({'checkin':y[i],'score':commentsentiment(s)},ignore_index=True)
datascore.to_csv('datascore.csv', encoding='utf_8_sig')
x=[datetime.strptime(date, "%Y-%m-%d").date() for date in datascore['checkin']]
print(x)
